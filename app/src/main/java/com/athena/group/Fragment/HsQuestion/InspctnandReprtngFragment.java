 ...
 */
public class InspctnandReprtngFragment extends Fragment {
    ...
    public InspctnandReprtngFragment() {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        toolbar_Title.setText(arrayList.get(0).getInspectionAndReporting().get(0).getHsLabel());
...
*/
        ...
        btn_next.setOnClickListener(view -> {
            ...
            Log.e("TAG", "questiondataList: 2-- " + questiondataList.size());
            ...
            for (int i = 0; i < questiondataList.size(); i++) {
                ...
                if (!questiondataList.get(i).getAns_ques_value().equals("")) {
                    Utility.getAppcon().getSession().check_valid2 = "yes";
                    //callNextApi();
                } else {
                    Utility.getAppcon().getSession().check_valid2 = "no";
                    //callNextApi();
                    break;
                }
            }

            if (Utility.getAppcon().getSession().check_valid2.equals("no")) {
                callNextApi();
            } else {
                callNextApi();
            }
...
//                    questiondataList.get(0).setAns_ques_value(questiondataList1.get(0).getAns_ques_value());
//                    questiondataList.get(0).setAns_ques_value_no(questiondataList1.get(0).getAns_ques_value_no());
                        Utility.getAppcon().getSession().check_valid2 = "yes";
                        callNextApi();
                    }
                } else {
        ...
        for (int i = 0; i < arrayList.get(0).getInspectionAndReporting().size(); i++) {
            CommanModel commanModel = new CommanModel();
            commanModel.setHsId(arrayList.get(0).getInspectionAndReporting().get(i).getHsId());
            commanModel.setHsLabel(arrayList.get(0).getInspectionAndReporting().get(i).getHsLabel());
            commanModel.setHsQues(arrayList.get(0).getInspectionAndReporting().get(i).getHsQues());
        ...
        adapter = new QuestionAdapter(getActivity(), commanModelslist, "inspctnreporting", new QuestionAdapter.OnItemClickListener() {
            @Override
    ...
    private void callNextApi1() {
        ...
        questiondataList.clear();
        Observable<DemoModel> responseObservable = apiservice.back_hsquestion_topic_data(Utility.getAppcon().getSession().data_value, "HSEQ Inspections");
        responseObservable.subscribeOn(Schedulers.newThread())
                .observeOn(rx.android.schedulers.AndroidSchedulers.mainThread())
                .onErrorResumeNext(throwable -> {
                    if (throwable instanceof retrofit2.HttpException) {
...
*/
                            ...
                            Log.e("TAG", "back click ans=-- " + new Gson().toJson(Utility.getAppcon().getSession().questiondataList1));
