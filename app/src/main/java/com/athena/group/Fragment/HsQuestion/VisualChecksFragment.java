 ...
 */
public class VisualChecksFragment extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        toolbar_Title.setText(arrayList.get(0).getVisualChecks().get(0).getHsLabel());
        txt_date.setText(Utility.date);
        txtdate = Utility.date;

        List<Example> arrayListQuestion = Utility.getAppcon().getSession().questiondataList18;
        ...
        for (int i = 0; i < arrayList.get(0).getVisualChecks().size(); i++) {
            CommanModel commanModel = new CommanModel();
            commanModel.setHsId(arrayList.get(0).getVisualChecks().get(i).getHsId());
            commanModel.setHsLabel(arrayList.get(0).getVisualChecks().get(i).getHsLabel());
            commanModel.setHsQues(arrayList.get(0).getVisualChecks().get(i).getHsQues());
        ...
        adapter = new QuestionAdapter(getActivity(), commanModelslist, "VisualChecks", new QuestionAdapter.OnItemClickListener() {
            @Override
...
callNextApi1();
        ...
        btn_next.setOnClickListener(view -> {
            ...
            Utility.getAppcon().getSession().questiondataList18 = examples1;
            ...
            for (int i = 0; i < questiondataList.size(); i++) {
                ...
                if (!questiondataList.get(i).getAns_ques_value().equals("")) {
                    Utility.getAppcon().getSession().check_valid18 = "yes";
                    //callNextApi();
                } else {
                    Utility.getAppcon().getSession().check_valid18 = "no";
                    //callNextApi();
                    break;
                }
            }

            if (Utility.getAppcon().getSession().check_valid18.equals("no")) {
                callNextApi();
            } else {
                callNextApi();
            }
...
//        view.requestFocus();
    ...
    private void callNextApi1() {
        ...
        Observable<DemoModel> responseObservable = apiservice.back_hsquestion_topic_data(Utility.getAppcon().getSession().data_value,arrayList.get(0).getMaterials().get(0).getHsLabel());
        responseObservable.subscribeOn(Schedulers.newThread())
                .observeOn(rx.android.schedulers.AndroidSchedulers.mainThread())
                .onErrorResumeNext(throwable -> {
                    if (throwable instanceof retrofit2.HttpException) {
