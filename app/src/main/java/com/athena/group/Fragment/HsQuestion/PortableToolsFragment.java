 ...
 */
public class PortableToolsFragment extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        toolbar_Title.setText(arrayList.get(0).getPortableTools().get(0).getHsLabel());
        txt_date.setText(Utility.date);
        txtdate = Utility.date;

        List<Example> arrayListQuestion = Utility.getAppcon().getSession().questiondataList12;
        ...
        for (int i = 0; i < arrayList.get(0).getPortableTools().size(); i++) {
            CommanModel commanModel = new CommanModel();
            commanModel.setHsId(arrayList.get(0).getPortableTools().get(i).getHsId());
            commanModel.setHsLabel(arrayList.get(0).getPortableTools().get(i).getHsLabel());
            commanModel.setHsQues(arrayList.get(0).getPortableTools().get(i).getHsQues());
        ...
        adapter = new QuestionAdapter(getActivity(), commanModelslist, "PortableTools", new QuestionAdapter.OnItemClickListener() {
            @Override
...
*/
        ...
        btn_next.setOnClickListener(view -> {
            ...
            Utility.getAppcon().getSession().questiondataList12 = examples1;
            ...
            for (int i = 0; i < questiondataList.size(); i++) {
                ...
                if (!questiondataList.get(i).getAns_ques_value().equals("")) {
                    Utility.getAppcon().getSession().check_valid12 = "yes";
                    //callNextApi();
                } else {
                    Utility.getAppcon().getSession().check_valid12 = "no";
                    //callNextApi();
                    break;
                }
            }
...
//        view.requestFocus();
    ...
    private void callNextApi1() {
        ...
        Observable<DemoModel> responseObservable = apiservice.back_hsquestion_topic_data(Utility.getAppcon().getSession().data_value,arrayList.get(0).getVehiclesVehicleMovementsPlantAndMachinery().get(0).getHsLabel());
        responseObservable.subscribeOn(Schedulers.newThread())
                .observeOn(rx.android.schedulers.AndroidSchedulers.mainThread())
                .onErrorResumeNext(throwable -> {
                    if (throwable instanceof retrofit2.HttpException) {
