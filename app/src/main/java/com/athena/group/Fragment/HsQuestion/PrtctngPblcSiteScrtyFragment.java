 ...
 */
public class PrtctngPblcSiteScrtyFragment extends Fragment {
    ...
    public PrtctngPblcSiteScrtyFragment() {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        toolbar_Title.setText(arrayList.get(0).getProtectingThePublicAndSiteSecurity().get(0).getHsLabel());
        txt_date.setText(Utility.date);
        txtdate = Utility.date;

        List<Example> arrayListQuestion = Utility.getAppcon().getSession().questiondataList15;
        ...
        for (int i = 0; i < arrayList.get(0).getProtectingThePublicAndSiteSecurity().size(); i++) {
            CommanModel commanModel = new CommanModel();
            commanModel.setHsId(arrayList.get(0).getProtectingThePublicAndSiteSecurity().get(i).getHsId());
            commanModel.setHsLabel(arrayList.get(0).getProtectingThePublicAndSiteSecurity().get(i).getHsLabel());
            commanModel.setHsQues(arrayList.get(0).getProtectingThePublicAndSiteSecurity().get(i).getHsQues());
        ...
        adapter = new QuestionAdapter(getActivity(), commanModelslist, "prtctngpblcscrty", new QuestionAdapter.OnItemClickListener() {
            @Override
        ...
        btn_next.setOnClickListener(view -> {
            ...
            Utility.getAppcon().getSession().questiondataList15 = examples1;
            ...
            for (int i = 0; i < questiondataList.size(); i++) {
                ...
                if (!questiondataList.get(i).getAns_ques_value().equals("")) {
                    Utility.getAppcon().getSession().check_valid15 = "yes";
                    //callNextApi();
                } else {
                    Utility.getAppcon().getSession().check_valid15 = "no";
                    //callNextApi();
                    break;
                }
            }

            if (Utility.getAppcon().getSession().check_valid15.equals("no")) {
                callNextApi();
            } else {
                callNextApi();
            }
...
//        view.requestFocus();
    ...
    private void callNextApi1() {
        ...
        Observable<DemoModel> responseObservable = apiservice.back_hsquestion_topic_data(Utility.getAppcon().getSession().data_value,arrayList.get(0).getExcavations().get(0).getHsLabel());
        responseObservable.subscribeOn(Schedulers.newThread())
                .observeOn(rx.android.schedulers.AndroidSchedulers.mainThread())
                .onErrorResumeNext(throwable -> {
                    if (throwable instanceof retrofit2.HttpException) {
