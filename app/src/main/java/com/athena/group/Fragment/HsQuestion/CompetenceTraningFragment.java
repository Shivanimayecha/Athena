...
public class CompetenceTraningFragment extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        toolbar_Title.setText(arrayList.get(0).getCompetenceTraining().get(0).getHsLabel());
        txt_date.setText(Utility.date);
        txtdate = Utility.date;

        List<Example> arrayListQuestion = Utility.getAppcon().getSession().questiondataList4;
        ...
        adapter = new QuestionAdapter(getActivity(), commanModelslist, "competencetraning", new QuestionAdapter.OnItemClickListener() {
            @Override
...
*/
      ...
      //  questiondataList1 = questiondataList;
        btn_next.setOnClickListener(view -> {
            ...
            Utility.getAppcon().getSession().questiondataList4 = examples1;
            ...
            for (int i = 0; i < questiondataList.size(); i++) {
                ...
                if (!questiondataList.get(i).getAns_ques_value().equals("")) {
                    Utility.getAppcon().getSession().check_valid4 = "yes";
                    //callNextApi();
                } else {
                    Utility.getAppcon().getSession().check_valid4 = "no";
                    //callNextApi();
                    break;
                }
            }

            if (Utility.getAppcon().getSession().check_valid4.equals("no")) {
                callNextApi();
            } else {
                callNextApi();
            }
...
//        view.requestFocus();
    ...
    private void callNextApi1() {
        ...
        Observable<DemoModel> responseObservable = apiservice.back_hsquestion_topic_data(Utility.getAppcon().getSession().data_value,arrayList.get(0).getDocumentation().get(0).getHsLabel());
        responseObservable.subscribeOn(Schedulers.newThread())
                .observeOn(rx.android.schedulers.AndroidSchedulers.mainThread())
                .onErrorResumeNext(throwable -> {
                    if (throwable instanceof retrofit2.HttpException) {
...
*/
                            ...
                            Utility.getAppcon().getSession().questiondataList3 = examples;
